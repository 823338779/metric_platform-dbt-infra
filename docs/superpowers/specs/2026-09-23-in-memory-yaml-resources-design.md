...
状态：用户已于 2026-09-23 确认，包含缺失/空白回退规则；进入实施计划阶段，尚未开始实现。
...
- 尽量少侵入已有代码：新增行为优先放入适配模块，采用组合复用现有实现，不以引入设计模式为理由重构既有流程。
...
采用组合式适配。原始命令构造器继续负责白名单和 argv 生成，外层资源命令适配器在有有效 resources 时把生成的命令转换为 worker 调用；无有效资源时直接返回原命令。worker 内调用原始构造器，不调用外层适配器，避免递归。

现有模块的变更限于：

- `models.py`：两个请求模型增加 resources 字段并调用独立校验函数；CommandSpec 增加向后兼容的可选 stdin 和任务产物标记。worker 私有协议放入新模块。
- `api.py`：替换为资源命令适配器入口，增加有效资源分支的前置检查选择，注册请求体限制中间件及必要生命周期调用；不把解析和文件管理逻辑放进路由。
- `jobs.py`：增加可选 stdin 传输及产物管理 helper 的调用；保留状态机、项目锁、任务保留、脱敏和进程树处理，不复制或另建 JobRunner。
- `settings.py`、`main.py`：任务产物根目录的配置和注入。
- `commands.py`、`projects.py` 保持实现不变。adapter 白名单常量提取为共享小模块，API 和 worker 复用同一份值，不改原判定规则。
- 相关测试、README 和 AGENTS.md。

新增职责边界：`resource_commands.py` 适配既有命令，`resource_protocol.py` 定义 worker 私有输入，`resource_adapter.py` 隔离上游内部 API，`resource_worker.py` 负责进程入口，`job_artifacts.py` 管理产物，`request_limits.py` 承担请求体上限，`resources.py` 负责输入校验。使用函数或小型对象满足实际接口即可，不额外建立抽象工厂、插件注册表或通用策略框架。

审查时逐行说明已有文件的修改为何是接入该功能所必需；禁止相邻整理、重命名或格式重排。通过旧命令构造测试和空 resources 的等价性测试证明默认行为未被改写。
...
14. 无有效 resources 时，资源命令适配器直接委托原命令构造器并返回原 CommandSpec；原命令构造与项目解析模块无实现改动，既有生命周期测试保持通过。
