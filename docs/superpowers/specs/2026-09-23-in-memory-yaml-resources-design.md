...
在现有 dbt 和 MetricFlow 任务请求中增加可选字段 `resources: dict[str, str]`。key 是外部 YAML 文件名，value 是 UTF-8 YAML 原文。解析项目时，同名资源优先使用本次请求的内存内容，未提供的资源仍使用基础项目。
...
- 非空 resources 适用于 `parse`、`compile`、`seed`、`run`、`test`、`build` 和现有四种 MetricFlow 命令。`debug` 不消费 schema 定义，非空 resources 返回 422，防止静默忽略。
- 非空白但无效的 YAML 在 worker 中按 dbt 解析错误失败。空或纯空白 value 在请求校验阶段拒绝；本接口不提供删除文件语义。
- 第一阶段采用模块常量限制：最多 100 个资源，单项原文 UTF-8 大小不超过 1 MiB，总原文大小不超过 5 MiB，文件名不超过 128 个字符。两端校验相同规则；边界值测试覆盖这些限制。
...
```text
HTTP 请求
  ├─ resources 为空 → 原有 dbt/mf CLI 与行为
  └─ resources 非空
       → 校验并排队
...
- 第一阶段非空 resources 始终完整解析，禁止复用和写入 `partial_parse.msgpack`。必须显式隔离其写入点，不能只依赖 `--no-partial-parse` 或 `--no-write-json`。
...
- 非空 resources 时，在同一 worker 内先执行 dbt parse，再运行请求指定的 mf 操作。调用方无需提前生成基础项目 manifest。
- 解析产物写入当前任务目录；从本次 manifest 获取 adapter 类型，继续应用现有 MetricFlow adapter 白名单。
- 原来基于项目 `target/manifest.json` 的同步预检查仅用于无 resources 路径；resources 路径的适配器检查转移到 worker。此时不支持的 adapter 体现为已接受任务的失败，而非提交时的 HTTP 422。
...
非空 resources 模式关闭 dbt/mf 文件日志，诊断经 stdout/stderr 返回并沿用服务脱敏。对于可能包含原文的解析异常，只输出资源名、行列和整理后的错误类别/消息，不直接转储请求或异常中的 YAML 源码片段。HTTP 请求体不写日志。
...
| 场景 | 行为 |
| --- | --- |
| resources 缺省或为空 | 保持当前 HTTP/CLI 行为，包括现有 manifest 前置要求 |
| 请求体超限 | HTTP 413，不启动任务 |
| 字段类型、资源名、数量/大小非法，debug 携带非空 resources | HTTP 422，不启动任务 |
...
未传 resources 的任务不受内存适配 hook、目标路径或缓存策略影响。现有 StarRocks 的 dbt 能力保留，MetricFlow 限制不变。
...
- `api.py`：请求体上限及有/无 resources 的提交分支。
...
1. 不传 resources 和 `{}` 时原有 dbt/mf API 测试全部通过。
2. 内存同名 YAML 改变解析后的模型/指标语义，磁盘源文件逐字节不变。
3. 磁盘同名 YAML 无效、内存 YAML 有效时成功，证明覆盖发生在解析之前。
4. 新增虚拟 YAML 可被发现；未提供资源从基础项目读取；未知 ref 仍失败。
5. 名称歧义、大小写冲突、路径输入、忽略规则、重复定义、非字符串值、空文本及上限边界按契约处理。
...
10. `parse`、`compile`、`seed`、`run`、`test`、`build` 的代表性 DuckDB 测试消费内存定义，debug 明确拒绝；四种 mf 命令验证当前语义来源和现有输出契约。
...
本次评审确认：文件名匹配规则、仅请求内生效、原文/缓存不落盘与派生产物可落盘的边界、限定内部 API 例外、debug 的拒绝语义，以及 resources 路径中适配器错误发生于异步任务阶段。
